...
Runtime.Context = class extends Runtime.CoreObject{
	...
	constructor(){
		super();
		this._entities = new Runtime.Vector();
		this._modules = new Runtime.Vector();
		this._providers = new Runtime.Map();
		this._providers_obj = new Runtime.Map();
	}
	/**
	 * Destructor
	 */
	...
	createProvider(provider_name){
		if (this._providers.has(provider_name)){
			...
			var obj = Runtime.rtl.newInstance(info.value);
			if (info.init){
				var f = info.init;
				obj = f(this, obj);
			}
			else {
				var f = Runtime.rtl.method(info.value, "init");
				obj = f(this, obj);
			}
	...
	registerModule(module_name){
		...
		if (!Runtime.rtl.class_exists(module_description_class_name)){
			return this;
		}
		this._modules.push(module_name);
		/* Register required Modules*/
		var modules = Runtime.rtl.callStaticMethod(module_description_class_name, "requiredModules", (new Runtime.Vector()));
		if (modules != null){
		...
		var entities = Runtime.rtl.callStaticMethod(module_description_class_name, "entities", (new Runtime.Vector()).push(this));
		if (entities != null){
			this._entities = this._entities.appendVector(entities);
		}
		Runtime.rtl.callStaticMethod(module_description_class_name, "onRegister", (new Runtime.Vector()).push(this));
		return this;
	}
	/**
	 * Apply Lambda Chain
	...
	chain(filter_name, obj){
		...
		var entities = this._entities.filter((item) => {
			return item instanceof Runtime.LambdaChain;
		});
	...
	readConfig(config){
		...
		if (runtime != null && runtime instanceof Runtime.Dict){
	...
	init(){
		...
		var providers = this._entities.filter((item) => {
			return item instanceof Runtime.Provider;
		});
		...
		for (var i = 0; i < sz; i++){
			var module_name = this._modules.item(i);
			var module_description_class_name = Runtime.rtl.toString(module_name)+".ModuleDescription";
			Runtime.rtl.callStaticMethod(module_description_class_name, "onInitContext", args);
		}
		return this;
	}
	/**
