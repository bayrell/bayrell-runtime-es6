...
Object.assign(Runtime.rs,
{
	...
	{
		var arr1 = Runtime.rs.explode(__ctx, ".", filepath).toVector(__ctx);
		var arr2 = Runtime.rs.explode(__ctx, "/", filepath).toVector(__ctx);
		var ret = new Runtime.PathInfo(__ctx);
		ret.filepath = filepath;
		ret.extension = arr1.pop(__ctx);
		ret.basename = arr2.pop(__ctx);
		ret.dirname = Runtime.rs.implode(__ctx, "/", arr2);
		var ext_length = Runtime.rs.strlen(__ctx, ret.extension);
		if (ext_length > 0)
		{
			ext_length++;
		}
	...
	filename: function(__ctx, filepath)
	{
		...
		if (ext != "")
		{
			var sz = 0 - Runtime.rs.strlen(__ctx, ext) - 1;
	...
	{
		if (ch == undefined) ch = "/";
		var source = Runtime.rs.explode(__ctx, ch, filepath);
		var base = Runtime.rs.explode(__ctx, ch, basepath);
