...
Object.assign(Runtime.fs,
{
	...
	{
		return Runtime.re.replace("//", "/", this.DIRECTORY_SEPARATOR + Runtime.rtl.toStr(s));
	},
	/**
	 * Add last slash
	 */
	...
	{
		return Runtime.re.replace("//", "/", s + Runtime.rtl.toStr(this.DIRECTORY_SEPARATOR));
	},
	/**
	 * Concat
	 */
	...
	exists: async function(path, chroot)
	{
		...
		if (chroot != "" && Runtime.rs.substr(chroot, -1) != "/")
		{
	...
	symlink: async function(target, link_name, chroot)
	{
		...
		if (Runtime.rs.substr(target_path, 0, 2) != "..")
		{
			target_path = chroot + Runtime.rtl.toStr(target);
		}
		if (Runtime.rs.substr(link_name_path, 0, 2) != "..")
		{
	...
	readDirectoryRecursive: async function(dirname, chroot, parent_name)
	{
		...
		for (var i = 0;i < items.count();i++)
		{
			...
			var item_name2 = Runtime.fs.concat(parent_name, item_name);
